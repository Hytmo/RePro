  ...
  return (
    <div className="mt-8 space-y-6">
      <section className="repro-card rounded-lg p-6">
        <h2 className="font-black text-ink">{labels.exportTitle}</h2>
        <p className="mt-1 text-sm text-ink-soft">{labels.exportText}</p>
        <button onClick={exportData} disabled={busy} className="mt-3 rounded-full border border-border bg-white px-4 py-2 text-sm font-bold text-ink-soft transition hover:border-brand-400 disabled:opacity-60">{labels.exportButton}</button>
      </section>
      <section className="rounded-lg border border-red-200 bg-red-50 p-6">
        <h2 className="font-black text-red-800">{labels.dangerTitle}</h2>
        <p className="mt-1 text-sm text-red-700">{labels.deleteText}</p>
        <button onClick={deleteAccount} disabled={busy} className="mt-3 rounded-full bg-red-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-red-700 disabled:opacity-60">{labels.deleteButton}</button>
      </section>
    </div>
  );
}
