          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {cats.map((c: any) => (
              <Link
                ...
                className="rounded-full bg-surface-2 px-3 py-1 text-xs font-medium text-ink-soft hover:text-brand-700"
              >
        ...
        <div className="lg:col-span-2">
          ...
          {company.rating_count > 0 && (
            <section className="mb-8 rounded-2xl border border-border bg-background p-6">
              ...
              <RatingBars
                labels={{
                  quality: t('criteria.quality'),
                  value: t('criteria.value'),
                  communication: t('criteria.communication'),
                  punctuality: t('criteria.punctuality')
                }}
          ...
          <section>
            <h2 className="mb-4 text-lg font-semibold text-ink">
              {t('reviewsTitle')} <span className="text-muted">({reviews.length})</span>
            </h2>
              ...
              {reviews.map((rv: any) => {
                ...
                return (
                  <article key={rv.id} className="rounded-2xl border border-border bg-background p-5">
                    ...
                    {rv.helpful_count > 0 && (
                      <p className="mt-3 text-xs text-muted">{tr('helpfulCount', {count: rv.helpful_count})}</p>
                    )}
                    {resp && (
            ...
            <p className="mt-6 rounded-xl border border-dashed border-border bg-surface px-4 py-3 text-center text-sm text-muted">
              {t('writeReviewNote')}
            </p>
          </section>
        </div>
        ...
        <aside className="space-y-6">
          <section className="rounded-2xl border border-border bg-background p-6">
            ...
            <ul className="space-y-2 text-sm">
              {company.phone && (
                <li className="flex items-center gap-2 text-ink-soft"><i className="ti ti-phone" aria-hidden="true" />{company.phone}</li>
              )}
              {company.email && (
                <li className="flex items-center gap-2 text-ink-soft"><i className="ti ti-mail" aria-hidden="true" />{company.email}</li>
              )}
              {company.website && (
                <li className="flex items-center gap-2"><i className="ti ti-world text-ink-soft" aria-hidden="true" /><a href={company.website} target="_blank" rel="noopener noreferrer" className="text-brand-700 hover:underline">{t('website')}</a></li>
              )}
              {company.address && (
                <li className="flex items-start gap-2 text-ink-soft"><i className="ti ti-map-pin mt-0.5" aria-hidden="true" /><span>{company.address}{company.postal_code ? `, ${company.postal_code}` : ''} {company.city}</span></li>
              )}
            </ul>
          </section>
            ...
            <section className="rounded-2xl border border-border bg-background p-6">
              ...
              <table className="w-full text-sm">
                <tbody>
                  {DAYS.map((d) => (
                    <tr key={d}>
                      <td className="py-1 text-ink-soft">{t(`days.${d}`)}</td>
                      <td className="py-1 text-right text-ink">{!hours[d] || hours[d] === 'closed' ? t('closed') : hours[d]}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
